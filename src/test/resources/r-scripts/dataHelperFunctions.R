...
replaceMissingValues<- function(data) {
  data$workclass<-replace(data$workclass,which(is.na(data$workclass)), rep("too-cool-to-work", sum(is.na(data$workclass))))
  data$occupation<-replace(data$occupation,which(is.na(data$occupation)), rep("hedonist", sum(is.na(data$occupation))))
  data$native_country<-replace(data$native_country,which(is.na(data$native_country)), rep("Fiji", sum(is.na(data$native_country))))
  return(data)
}
